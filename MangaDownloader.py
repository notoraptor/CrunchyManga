...
class MangaDownloader():
    ...
    def xord(self, bytear, key):
        return bytearray(((x ^ ord(y)) for (x, y) in zip(bytear, cycle(key))))
        ...
        while (_config):
            try:
                with open('config.json') as config_file:
                    ...
                    if self.zip == True or self.zip == False:
                        pass
                    else:
                        self.config()
                    if self.d_volumes == True or self.d_volumes == False:
                        pass
                    else:
                        self.config()
                    if self.overwrite == True or self.overwrite == False:
                        pass
                    else:
                        self.config()
                    if self.delete_files == True or self.delete_files == False:
                        pass
                    else:
                        self.config()
                    _config = False
...
'''
    ...
    def login(self, usuario, password):
        ...
        logindata = {'formname': 'login_form', 'fail_url': 'http://www.crunchyroll.com/login',
                     'login_form[name]': usuario, 'login_form[password]': password, 'login_form[_token]': hidden,
    ...
    def CrunchyManga(self):
        ...
        if re.match(
                r"^(http:\/\/)(w{3}\.)?(crunchyroll\.com\/comics_read(\/(manga|comipo|artistalley))?\?(volume\_id|series\_id)\=[0-9]+&chapter\_num\=[0-9]+(\.[0-9])?)",
                self.url):  # Crunchyroll por episodios.
            try:
                html = self.download()
                soup = BeautifulSoup(html, features="lxml")
                self.manga_titulo = soup.find(u"span", {u"itemprop": u"title"}).text
                self.manga_titulo = self.manga_titulo.replace(':', ' ')
                manga = soup.find("object", {u"id": u"showmedia_videoplayer_object"}).find("embed", {
                    u"type": u"application/x-shockwave-flash"}).get("flashvars")
                manga = manga.split("=")
                n = len(manga) - 2
                ...
                print(
                    "The link is certainly from Crunchyroll, but it seems that it's not correct. Verify it and try again.")
                return
            ...
            if chapter_id == "":
                ...
                x = self.Directorio()
                if not x:
                    print("The folder %s - %s already exists and overwrite folders is deactivated." % (
                        self.manga_titulo, self.manga_numcap))
                    os.chdir(self.directorio)
                else:
                ...
                r"^(http:\/\/)(w{3}\.)?(crunchyroll\.com\/comics_read)(\/(manga|comipo|artistalley))?(\?volume\_id\=[0-9]+)$",
                self.url):  # Crunchyroll por volumenes
            volume = re.match(
                r"^(http:\/\/)(w{3}\.)?(crunchyroll\.com\/comics_read)(\/(?:manga|comipo|artistalley))?(\?volume\_id\=([0-9]+))$",
                self.url)
            try:
            ...
            try:
                ...
                print(
                    "To download volumes you need a premium account. Standard accounts can only download the latest chapter.")
                return
            c = 0
            cv = True
            rr = 0
            ...
            while c < len(capitulo):
                ...
                url_capitulo = "http://api-manga.crunchyroll.com/list_chapter?session_id=" + sesion_id + "&chapter_id=" + \
                               chapter_id[c] + "&auth=" + cr_auth
                html = self.download(url_capitulo)
                soup = json.loads(html)
                c2 = 0
                pages = {}
            ...
            if self.zip and self.manga_vol != "0" and rr > 0:
                ...
                r"^(http:\/\/)(w{3}\.)?(crunchyroll\.com\/comics\/(manga|comipo|artistalley)\/[a-z0-9\-]+\/)(volumes)$",
                self.url):  # Crunchyroll por serie entera
            ...
            if self.d_volumes:
                ...
                vol_id = []
                try:
                    if soup.find(u"li", {r"class": u"queue-item volume-simul"}).get(u"volume_id"):
                        volumen_id = soup.findAll(u"li", {r"class": re.compile(u"queue-item volume-")})
                ...
                i = 0
                for vols in vol_id:
                    print("Downloading all volumes and individual chapters available. Volumes: %d/%d" % (
                        i + 1, len(vol_id)))
                    self.url = "http://www.crunchyroll.com/comics_read/manga?volume_id=" + vols
                    self.CrunchyManga()
                    i += 1
            else:
                volumen_id = soup.find(u"li", {r"class": re.compile(u"queue-item volume-")}).get(u"volume_id")
                url_vol = "http://www.crunchyroll.com/comics_read/manga?volume_id=" + volumen_id
                html = self.download(url_vol)
                soup = BeautifulSoup(html)
                sesion_id = soup.find("object", {u"id": u"showmedia_videoplayer_object"}).find("embed", {
                    u"type": u"application/x-shockwave-flash"}).get("flashvars")
                ...
                try:
                    ...
                    print(
                        "To download complete series you need a premium account. Standard accounts can only download the latest chapter.")
                    return
                c = 0
                while c < len(capitulo):
                    x = capitulo[c][-2:]
                    if x == "00":
                        ...
                        capitulo[c] = capitulo[c][:-1]
                    self.manga_numcap = capitulo[c]
                    print("\n\n%s chapters %d of %d: Downloading chapter %s" % (
                        self.manga_titulo, c + 1, len(capitulo), self.manga_numcap))
                    url_capitulo = "http://api-manga.crunchyroll.com/list_chapter?session_id=" + sesion_id + "&chapter_id=" + \
                                   chapter_id[c] + "&auth=" + cr_auth
                    html = self.download(url_capitulo)
                    soup = json.loads(html)
                    ...
                    cc = 1
                    if not x:
                        print("\nThe folder %s - %s already exists and overwrite folders is deactivated." % (
                            self.manga_titulo, self.manga_numcap))
                    else:
                        descarga = self.downloadPages(pages)
                        cc += 1
...
if __name__ == '__main__':
    ...
    parser.add_argument("-u", "--url", type=str,
                        help="Crunchyroll Link. if you get an error, try using double quotation marks (\")")
    parser.add_argument("-l", "--login", nargs=2,
                        help="Crunchyroll login: -l User password. if your password has a blank, use double quotation marks (\"). Example: \"This is a password.\"")
